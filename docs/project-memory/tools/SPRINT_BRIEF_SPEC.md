...
- Keep it concise — agents receive Goal and Constraints on top of their own brief (Merge Order and Merge Verification are not distributed to agents).
...
4. **Per-agent brief**: Goal and Constraints from the meta block are prepended, then the agent's own section content — written to `AGENT_BRIEF.md` in each worktree (Merge Order and Merge Verification are NOT distributed to agents)
